...
async function save(key, val) {
  try { await window.storage.set(key, JSON.stringify(val)); } catch (e) { console.warn("save failed", e); }
}
...
function Btn({ children, onClick, color = C.neonBlue, outline = false, small = false, style = {}, disabled = false }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      fontSize: small ? 12 : 14, boxShadow: (!outline && !disabled) ? `0 0 14px ${color}44` : "none",
...
function LoadingScreen() {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
      ...
      <div style={{ fontSize: 28, fontWeight: 900, color: C.neonBlue, letterSpacing: 6 }}>FRIDAY</div>
      <div style={{ color: C.textSecondary, fontSize: 14 }}>Loading your profile…</div>
      <div style={{ width: 40, height: 40, border: `3px solid ${C.border}`, borderTop: `3px solid ${C.neonBlue}`, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
...
function SetupWizard({ onComplete }) {
  ...
  const steps = [
    {
      ...
      fields: (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Input label="Your Name" value={form.name} onChange={v => set("name", v)} placeholder="e.g. Arjun" />
          <div style={{ display: "flex", gap: 10 }}>
            <Input label="Age" value={form.age} onChange={v => set("age", Number(v))} type="number" style={{ flex: 1 }} />
            <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <Input label="Height (cm)" value={form.height} onChange={v => set("height", Number(v))} type="number" style={{ flex: 1 }} />
            <Input label="Weight (kg)" value={form.weight} onChange={v => set("weight", Number(v))} type="number" style={{ flex: 1 }} />
          </div>
        </div>
      )
    },
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
              {[["Build Muscle", "💪"], ["Lose Fat", "🔥"], ["Gain Weight", "⬆️"], ["Maintain Health", "❤️"]].map(([g, ic]) => (
                ...
                }}><div style={{ fontSize: 20 }}>{ic}</div>{g}</div>
              ))}
            </div>
          </div>
          <Input label="Target Weight (kg)" value={form.targetWeight} onChange={v => set("targetWeight", Number(v))} type="number" />
        </div>
      )
    },
    {
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              {[["Beginner", "🌱"], ["Intermediate", "🔥"], ["Advanced", "⚡"]].map(([e, ic]) => (
                <div key={e} onClick={() => set("workoutExperience", e)} style={{
                  ...
                  fontWeight: 600, fontSize: 12, transition: "all 0.2s"
                }}><div style={{ fontSize: 18 }}>{ic}</div>{e}</div>
              ))}
            </div>
          </div>
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              {[["Vegetarian", "🥗"], ["Non-Veg", "🍗"], ["Vegan", "🌿"]].map(([fp, ic]) => (
                <div key={fp} onClick={() => set("foodPreference", fp)} style={{
                  ...
                  fontWeight: 600, fontSize: 12, transition: "all 0.2s"
                }}><div style={{ fontSize: 18 }}>{ic}</div>{fp}</div>
              ))}
            </div>
          </div>
          <div>
  ...
  const finish = async () => {
    ...
    await save("friday_profile", profile);
    onComplete(profile);
  };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 460 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ color: C.textSecondary, fontSize: 13 }}>Your AI Fitness Partner — Setup (one time only!)</div>
        </div>
        ...
        <Card>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
            ...
            <Btn onClick={isLast ? finish : () => setStep(s => s + 1)} style={{ flex: 1 }} disabled={saving}>
              {saving ? "Saving…" : isLast ? "🚀 Launch FRIDAY!" : "Next →"}
            </Btn>
          </div>
        </Card>
      </div>
...
export default function FridayApp() {
  ...
  useEffect(() => {
    (async () => {
      const prof = await load("friday_profile");
      if (!prof) { setAppState("setup"); return; }
      setProfile(prof);

      const key = todayKey();
      const stored = await load(`friday_log_${key}`);
      setTodayLog(stored || { calories: 0, protein: 0, carbs: 0, fats: 0, water: 0, meals: [], workoutDone: false, completedSets: {} });

      const storedTasks = await load("friday_tasks") || [];
      setTasks(storedTasks);

      const storedChat = await load("friday_chat");
      if (storedChat?.length) setChatMsgs(storedChat);

      setAppState("app");
    })();
  }, []);

  // ── Auto-save log whenever it changes
  useEffect(() => {
    if (todayLog && appState === "app") save(`friday_log_${todayKey()}`, todayLog);
  }, [todayLog]);

  // ── Auto-save tasks
  useEffect(() => {
    if (appState === "app") save("friday_tasks", tasks);
  }, [tasks]);

  // ── Auto-save chat
  useEffect(() => {
    if (appState === "app") save("friday_chat", chatMsgs.slice(-30));
  }, [chatMsgs]);

  const updateProfile = async (p) => {
    setProfile(p);
    await save("friday_profile", p);
  };

  const resetAll = async () => {
    if (!confirm("Reset everything? This cannot be undone.")) return;
    try { await window.storage.delete("friday_profile"); } catch {}
    try { await window.storage.delete("friday_tasks"); } catch {}
    try { await window.storage.delete("friday_chat"); } catch {}
    setAppState("setup");
    setProfile(null);
  };

  if (appState === "loading") return <LoadingScreen />;
  if (appState === "setup") return <SetupWizard onComplete={p => { setProfile(p); setTodayLog({ calories: 0, protein: 0, carbs: 0, fats: 0, water: 0, meals: [], workoutDone: false, completedSets: {} }); setAppState("app"); }} />;
  if (!todayLog) return <LoadingScreen />;
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", color: C.textPrimary }}>
      ...
      {/* Top Bar */}
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "12px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 100 }}>
        <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ color: C.textPrimary, fontWeight: 700, fontSize: 14 }}>{profile.name || "Warrior"}</div>
            <Tag color={C.neonGreen}>{profile.goal}</Tag>
          </div>
          <button onClick={resetAll} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, color: C.textMuted, fontSize: 11, padding: "4px 8px", cursor: "pointer", fontFamily: "inherit" }}>Reset</button>
        </div>
      </div>

      {/* Content */}
      <div style={{ padding: "18px 16px", maxWidth: 680, margin: "0 auto", paddingBottom: 90 }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: C.surface, borderTop: `1px solid ${C.border}`, display: "flex", padding: "6px 0 10px", zIndex: 100 }}>
        {tabs.map(t => {
          ...
          const pendingTasks = t.id === "tasks" ? tasks.filter(tk => !tk.done).length : 0;
          return (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              flex: 1, background: "none", border: "none", cursor: "pointer",
              padding: "6px 2px", display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
              color: active ? C.neonBlue : C.textMuted, fontFamily: "inherit", transition: "color 0.2s", position: "relative"
            }}>
              {pendingTasks > 0 && (
                <div style={{ position: "absolute", top: 2, right: "calc(50% - 14px)", background: C.neonPink, color: "#fff", fontSize: 9, fontWeight: 900, borderRadius: 10, padding: "1px 5px", minWidth: 14, textAlign: "center" }}>{pendingTasks}</div>
              )}
...
function Dashboard({ profile, log, setLog, tasks, setTab }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Card glow={C.neonBlue}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: C.textPrimary, marginTop: 2 }}>{profile.name || "Warrior"} 💪</div>
      ...
      {/* Macro Rings */}
      <Card>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14, color: C.textPrimary }}>Today's Nutrition</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        {[
          ...
          { icon: "✅", val: `${doneTasks.length}/${tasks.length}`, label: "Tasks done", color: C.neonPurple },
        ].map(s => (
        ...
        <Card>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ color: C.neonPurple, fontWeight: 700, fontSize: 13 }}>{taskPct}%</div>
          ...
          {pendingTasks.length === 0 && <div style={{ color: C.neonGreen, textAlign: "center", fontSize: 13, padding: "8px 0" }}>🎉 All tasks done!</div>}
        </Card>
      )}
      ...
      {tasks.length === 0 && (
        <Card style={{ textAlign: "center", padding: 24 }}>
          ...
          <div style={{ color: C.textSecondary, fontSize: 14, marginBottom: 14 }}>No tasks yet. Add your daily plans!</div>
          <button onClick={() => setTab("tasks")} style={{ background: C.neonPurple + "22", border: `1px solid ${C.neonPurple}`, borderRadius: 10, color: C.neonPurple, padding: "10px 20px", cursor: "pointer", fontWeight: 700, fontSize: 13, fontFamily: "inherit" }}>+ Add Tasks</button>
        </Card>
      )}
...
function TasksScreen({ tasks, setTasks }) {
  ...
  const addTask = () => {
    ...
    const task = {
      ...
      date: newDate, priority: newPriority, category: newCategory,
      createdAt: new Date().toISOString(),
    };
    setTasks(t => [task, ...t]);
    setNewText(""); setNewDate(""); setNewPriority("Medium"); setNewCategory("Fitness");
    setShowAdd(false);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Card glow={C.neonPurple}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <Ring pct={donePct} color={C.neonPurple} size={70} label="Done" value={`${donePct}%`} />
        </div>
      ...
      {showAdd && (
        <Card style={{ border: `1px solid ${C.neonPurple}44` }}>
          <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14, color: C.neonPurple }}>New Task</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <Input label="Task description" value={newText} onChange={setNewText} placeholder="e.g. Morning workout, Meal prep, Study…" />
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <Select value={newPriority} onChange={setNewPriority} options={priorities} />
              </div>
            </div>
            <div>
      ...
      {filtered.length === 0 ? (
        <Card style={{ textAlign: "center", padding: 32 }}>
          ...
          <div style={{ color: C.textSecondary, fontSize: 14 }}>No tasks here. Add something!</div>
        </Card>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map(task => (
            <Card key={task.id} style={{ padding: 14, border: task.done ? `1px solid ${C.border}` : `1px solid ${priorityColor[task.priority]}33`, opacity: task.done ? 0.65 : 1 }}>
              {editId === task.id ? (
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button onClick={() => { setEditId(task.id); setEditText(task.text); }}
                      style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, color: C.textMuted, fontSize: 13, padding: "4px 8px", cursor: "pointer", fontFamily: "inherit" }}>✏️</button>
                    <button onClick={() => deleteTask(task.id)}
                      style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, color: C.neonPink, fontSize: 13, padding: "4px 8px", cursor: "pointer", fontFamily: "inherit" }}>🗑</button>
                  </div>
                </div>
              )}
            </Card>
      ...
      {tasks.filter(t => t.done).length > 0 && (
        <button onClick={() => setTasks(t => t.filter(tk => !tk.done))}
          style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 10, color: C.textMuted, padding: "10px", cursor: "pointer", fontSize: 13, fontFamily: "inherit", width: "100%" }}>
...
function WorkoutScreen({ profile, log, setLog }) {
  ...
  const muscleColor = { Chest: C.neonBlue, Legs: C.neonGreen, Core: C.neonOrange, Biceps: C.neonPurple, Back: C.neonPink, Shoulders: "#00FFCC", Hamstrings: C.neonOrange };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Card glow={C.neonPurple}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 800 }}>{profile.workoutExperience} Plan</div>
            <div style={{ color: C.textSecondary, fontSize: 13, marginTop: 3 }}>{workouts.length} exercises · {totalSets} total sets</div>
          </div>
          <Ring pct={Math.round((doneSets / totalSets) * 100)} color={C.neonPurple} size={75} label="Sets Done" value={`${doneSets}/${totalSets}`} />
        </div>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 44, fontWeight: 900, color: running ? C.neonGreen : C.textMuted, fontVariantNumeric: "tabular-nums", letterSpacing: 2 }}>{fmt(elapsed)}</div>
          <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 10, flexWrap: "wrap" }}>
            <Btn onClick={() => setRunning(r => !r)} color={running ? C.neonOrange : C.neonGreen} small>{running ? "⏸ Pause" : "▶ Start Timer"}</Btn>
            <Btn onClick={() => { setRunning(false); setElapsed(0); }} outline color={C.textSecondary} small>↺ Reset</Btn>
            {!log.workoutDone
              ? <Btn onClick={() => { setRunning(false); setLog(l => ({ ...l, workoutDone: true })); alert("🎉 Workout complete! You're a beast!"); }} color={C.neonPink} small>Finish 💪</Btn>
              : <Tag color={C.neonGreen}>✓ Completed Today!</Tag>
            }
          </div>
        </div>
      </Card>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 5 }}>
                <Tag color={muscleColor[w.muscle] || C.neonBlue}>{w.muscle}</Tag>
                <Tag color={C.neonBlue}>{w.sets} × {w.reps}</Tag>
                <Tag color={C.textMuted}>⏱ {w.rest}</Tag>
              </div>
            </div>
          </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Card glow={C.neonBlue}>
        <div style={{ textAlign: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 40, fontWeight: 900, color: remaining >= 0 ? C.neonBlue : C.neonPink }}>{Math.abs(remaining)}</div>
          <div style={{ color: C.textSecondary, fontSize: 13 }}>{remaining >= 0 ? "calories remaining today" : "calories over goal ⚠️"}</div>
        </div>
        ...
        <div style={{ textAlign: "right", color: C.textMuted, fontSize: 11, marginTop: 4 }}>{Math.round(calPct)}% of {profile.dailyCalorieGoal} kcal goal</div>
      </Card>
      ...
      <Card>
        ...
        <input value={search} onChange={e => { setSearch(e.target.value); setSelected(null); }}
          placeholder="Search Indian foods… (Idli, Dal, Chicken…)"
        ...
        {selected && (
          <div style={{ marginTop: 12, background: C.neonBlue + "11", border: `1px solid ${C.neonBlue}44`, borderRadius: 10, padding: 14 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
              ...
              <span style={{ color: C.textMuted, fontSize: 12 }}>→ {Math.round(selected.cal * qty)} kcal · {+(selected.p * qty).toFixed(1)}g protein</span>
            </div>
      ...
      {log.meals.length > 0 && (
        <Card>
          ...
          {log.meals.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: i < log.meals.length - 1 ? `1px solid ${C.border}` : "none" }}>
              <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ color: C.neonBlue, fontWeight: 800, fontSize: 14 }}>{m.cal} kcal</div>
                <button onClick={() => removeFood(i)} style={{ background: "none", border: "none", color: C.neonPink, cursor: "pointer", fontSize: 14, padding: "2px 4px" }}>🗑</button>
              </div>
            </div>
          ))}
...
function ProgressScreen({ profile, updateProfile, log }) {
  ...
  const [history, setHistory] = useState(() => {
    const base = profile.weight;
    return [
      { d: "4 wks ago", w: +(base - 1.8).toFixed(1) },
      { d: "3 wks ago", w: +(base - 1.2).toFixed(1) },
      { d: "2 wks ago", w: +(base - 0.6).toFixed(1) },
      { d: "Last wk", w: +(base - 0.2).toFixed(1) },
      { d: "Today", w: base },
    ];
  });
  ...
  const logWeight = async () => {
    ...
    const updated = { ...profile, weight: w, bmi, bmiCategory: calcBMICategory(bmi) };
    await updateProfile(updated);
    setHistory(h => [...h.slice(-7), { d: "Now", w }]);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Card glow={C.neonGreen}>
        <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 16 }}>📊 Body Stats</div>
      ...
      {/* Weight Chart */}
      <Card>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 14 }}>📈 Weight Trend</div>
        <svg width="100%" viewBox={`0 0 ${svgW} ${svgH + 10}`}>
          <defs>
            <linearGradient id="lg" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor={C.neonBlue} stopOpacity="0.3" />
              <stop offset="100%" stopColor={C.neonBlue} stopOpacity="0" />
            </linearGradient>
          </defs>
          {history.map((h, i) => {
            if (i === 0) return null;
          ...
          {history.map((h, i) => {
            ...
            return (
              <g key={i}>
                ...
                <text x={x} y={y - 10} textAnchor="middle" fill={C.neonBlue} fontSize="9" fontWeight="700">{h.w}</text>
              </g>
            );
          })}
        </svg>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
          {history.map((h, i) => (
            <div key={i} style={{ textAlign: "center" }}>
              <div style={{ color: C.textMuted, fontSize: 10 }}>{h.d}</div>
            </div>
          ))}
        </div>
      </Card>
      ...
      {/* Log Weight */}
      <Card>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>⚖️ Log Weight</div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <span style={{ color: C.textSecondary, fontSize: 16 }}>kg</span>
...
function AIChat({ profile, msgs, setMsgs, log }) {
  ...
  const send = async (text) => {
    ...
    setLoading(true);
    const sys = `You are FRIDAY, an expert AI fitness & nutrition coach. User profile: Name: ${profile.name}, Age: ${profile.age}, Gender: ${profile.gender}, Weight: ${profile.weight}kg, Height: ${profile.height}cm, Goal: ${profile.goal}, Activity: ${profile.activityLevel}, Experience: ${profile.workoutExperience}, Food: ${profile.foodPreference}, BMI: ${profile.bmi} (${profile.bmiCategory}). Daily targets: ${profile.dailyCalorieGoal} kcal, ${profile.proteinGoal}g protein, ${profile.waterGoal}ml water. Today: ${log.calories} kcal eaten, ${log.protein}g protein, ${log.water}ml water, workout ${log.workoutDone ? "done" : "not done"}. Be specific, warm, motivating, give Indian-context advice. Keep replies under 120 words.`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: sys,
          messages: msgs.slice(-10).map(m => ({ role: m.role, content: m.text })).concat([{ role: "user", content: msg }])
        })
      });
      const data = await res.json();
      const reply = data.content?.[0]?.text || "Sorry, I couldn't respond right now. Try again!";
  ...
  const suggestions = ["What should I eat today?", "Give me a workout tip 💪", "How much protein do I need?", "Motivate me! 🔥", "Review my today's progress"];
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 180px)", minHeight: 400 }}>
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, paddingBottom: 12 }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", gap: 8 }}>
            {m.role === "assistant" && (
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: C.neonBlue + "33", border: `1.5px solid ${C.neonBlue}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0, marginTop: 2 }}>⚡</div>
            )}
            <div style={{
              maxWidth: "78%", padding: "10px 14px",
              borderRadius: m.role === "user" ? "16px 16px 4px 16px" : "16px 16px 16px 4px",
              background: m.role === "user" ? C.neonBlue : C.surface,
              border: `1px solid ${m.role === "user" ? C.neonBlue : C.border}`,
        ...
        {loading && (
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", background: C.neonBlue + "33", border: `1.5px solid ${C.neonBlue}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>⚡</div>
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: "16px 16px 16px 4px", padding: "10px 14px" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button onClick={() => send()} disabled={loading || !input.trim()} style={{
          background: (loading || !input.trim()) ? C.border : C.neonBlue, border: "none", borderRadius: 12,
          padding: "11px 16px", cursor: (loading || !input.trim()) ? "not-allowed" : "pointer",
          color: "#000", fontWeight: 700, fontSize: 15, fontFamily: "inherit",
          boxShadow: (!loading && input.trim()) ? `0 0 14px ${C.neonBlue}44` : "none", transition: "all 0.2s"
        }}>➤</button>
      </div>
    </div>
  );
